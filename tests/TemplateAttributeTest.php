...
class TemplateAttributeTest extends BaseAttributeTestCase
{
    ...
    {
        $errors = $this->analyzeTestFile('/data/FunctionTemplateAttribute.php');
        $this->assertCount(0, $errors);
    }

    public function testClassTemplateAttribute(): void
    {
        $errors = $this->analyzeTestFile('/data/ClassTemplateAttribute.php');
        $this->assertCount(0, $errors);
    }

    public function testInterfaceTemplateAttribute(): void
    {
        $errors = $this->analyzeTestFile('/data/InterfaceTemplateAttribute.php');
        $this->assertCount(0, $errors);
    }

    public function testTraitTemplateAttribute(): void
    {
        $errors = $this->analyzeTestFile('/data/TraitTemplateAttribute.php');
        $this->assertCount(0, $errors);
    }

    public function testMethodTemplateAttribute(): void
    {
        $errors = $this->analyzeTestFile('/data/MethodTemplateAttribute.php');
        $this->assertCount(0, $errors);
    }

    public function testInvalidMethodTemplateAttribute(): void
    {
        $errors = $this->analyzeTestFile('/data/InvalidMethodTemplateAttribute.php');
        $this->checkExpectedErrors($errors,[
            'Empty @template tag in docblock for test\PhpStaticAnalysis\PsalmPlugin\data\InvalidMethodTemplateAttribute::getName' => 11,
            'Argument 1 of PhpStaticAnalysis\Attributes\Template::__construct expects string, but 0 provided' => 11,
            'Empty @template tag in docblock for test\PhpStaticAnalysis\PsalmPlugin\data\InvalidMethodTemplateAttribute::getAnotherName' => 17,
