...
class IsReadOnlyAttributeTest extends BaseAttributeTestCase
{
    ...
    {
        $errors = $this->analyzeTestFile('/data/PropertyIsReadOnlyAttribute.php');
        $this->checkExpectedErrors($errors,[
            'test\PhpStaticAnalysis\PsalmPlugin\data\PropertyIsReadOnlyAttribute::$name is marked readonly' => 19,
        ]);
    }

    public function testInvalidPropertyIsReadOnlyAttribute(): void
    {
        $errors = $this->analyzeTestFile('/data/InvalidPropertyIsReadOnlyAttribute.php');
        $this->checkExpectedErrors($errors,[
